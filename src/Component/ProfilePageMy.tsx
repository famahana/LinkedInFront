...
function ProfilePageMy()
{
    ...
    const handleBannerUpload = async (e:ChangeEvent<HTMLInputElement>)=>
    {
        ...
        try
        {
            const updatedProfile = await profileService.uploadBanner(file);
            setProfile(updatedProfile);
        }
        catch(error)
        {
        ...
        <div className="container_profile_page">
            <div className="profile_banner" style={{backgroundImage: `url(${profile?.bannerUrl})`,}}>
                <div className="edit_profile_button_container">
                    ...
                    <button className="button_edit_banner" onClick={()=>bannerInputRef.current?.click()}>
            ...
            <div className="profile_content_container">
                <div className="profile_content">
                    <input ref={avatarInputRef}type="file"style={{ display: "none" }}accept="image/*"onChange={handleAvatarUpload}/>
                    <img onClick={()=> avatarInputRef.current?.click()} className="profile_avatar" src={profile?.avatarUrl} alt="" />
                </div>
            <div className="profile_info">
                {isEditing?(
                    <div>
                        <input className="profile_name" placeholder="first name" value={form.firstName} onChange={(e) => setForm({ ...form, firstName: e.target.value })}/>
                        <input className="profile_name" placeholder="last name" value={form.lastName} onChange={(e) => setForm({ ...form, lastName: e.target.value })}/>
                    </div>
                ): (
                ...
                {isEditing?(
                    <div>
                        <input className="profile_position" placeholder="company" value={form.company}onChange={(e) => setForm({ ...form, company: e.target.value })} />
                        <input className="profile_position" placeholder="position" value={form.position}onChange={(e) => setForm({ ...form, position: e.target.value })} />
                    </div>
                ):(
                ...
                {isEditing?(
                    <div>
                        <input className="profile_location" placeholder="location" value={form.location}onChange={(e) => setForm({ ...form, location: e.target.value })} />
                    </div>
                ):(
            ...
            <div className="profile_buttons">
                ...
                <button className="btn_secondary_2" onClick={logout}>Logout</button>  
            </div>
            </div>
        </div>
    )
